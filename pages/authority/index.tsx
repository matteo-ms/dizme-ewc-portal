...
export default function Home() {
  ...
  const { country, countryCode } = router.query as { country: string, countryCode: string };
  ...
  function prepareCredentialsToIssue(): CredentialToIssue[] {
    const countryCredentials = AvailableCredentials.filter(cred => {
      return cred.issuer.country === countryCode;
    
    })
    return countryCredentials
      .map((cred: AvailableCredential) => {
        return {
          ...cred,
          selected: false,
        };
